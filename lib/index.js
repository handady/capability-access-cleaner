...
//
// Registers two surfaces over the same cleanup.ps1:
//   1. an agent tool `clean_capability_access` (the model decides when to call it)
//   2. a slash command `/clean-capability-access` (a human clicks it in the `/`
//      palette; the handler runs the script DIRECTLY, no model involved)
...
function apply(ctx) {
  ...
  // Human-facing slash command: a user types/clicks `/clean-capability-access` in
  // the composer `/` palette and the handler runs cleanup.ps1 DIRECTLY — no model
  // turn, no AI conversation. Default is safe detection only; `fix` does the real
  // (UAC-gated) truncate, `selfTest` runs the no-admin demo.
  ctx.commands.register({
    name: "clean-capability-access",
    description: "一键检测/清理 CapabilityAccessManager 异常 WAL 文件（默认仅检测）",
    input: { hint: "[selfTest | fix]" },
    handler: async (invocation) => {
      const arg = invocation.rawInput.trim().toLowerCase();
      const flags = arg === "selftest" ? ["-SelfTest"] : arg === "fix" ? [] : ["-DryRun"];
      const { code, stdout, stderr } = await runScript(flags);
      const text = (stdout || stderr || "").trim();
      return { kind: code === 0 ? "success" : "error", text: text || "(no output)" };
    }
  });
}
